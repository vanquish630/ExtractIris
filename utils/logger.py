...
def setup_logger(work_dir=None, logfile_name='log.txt', logger_name='logger'):
  ...
  The function will set up a logger with `DEBUG` log level. Two handlers will
  be added to the logger automatically. One is the `sys.stdout` stream, with
  `INFO` log level, which will print important messages on the screen. The other
  ...
  Returns:
    ...
    logfile_name : The name of the file to save log messages. If logfile already
      exists, user can specify another in the prompt
  ...
  if os.path.isfile(logfile_name):
    ...
    while True:
      ...
      if decision == 'n':
        while True:
          new_logfile_name = input(f'Please specify another one.')
          new_logfile_name = new_logfile_name.strip().lower()
          logfile_name = new_logfile_name
          logger.info(f"changed name of log file to {logfile_name}")
      if decision == 'y':
